...
int main(){
    ...
    char outdir[]="propagate_pure_H_CGM/CGM_04_05pMpc/"; //////////////
    char fpath[200];
    char fname[100];
    if (stat(outdir, &st) == -1) {
        mkdir(outdir, 0700);
    ...
    int nc=1000; //////////////// 20x10kpc(nH0.0001)+200x1kpc(nH0.01)+140x40kpc(nH0.0001)
    double bkg[nc][6];
    double zuni=7;
    int no=12;
    ...
    for (int k=20; k<520; k++){
        for (int j=0; j<6; j++){
            bkg[k][j]=0;
        }
        double dr=0.0002; //////////////////
        double dist=0.4+dr*(k-20)+0.001;
        z7_cell(&cell[k], dist, dr);
        cell[k].nH=1e-2; ////////////
        cell[k].nHe=0;
        cell[k].T=1e4;
    ...
    for (int k=520; k<nc; k++){
        for (int j=0; j<6; j++){
            bkg[k][j]=0;
        }
        double dr=0.02; //////////////////
        double dist=0.5+dr*(k-520)+0.001;
        z7_cell(&cell[k], dist, dr);
        cell[k].nH=1e-4; ////////////
        cell[k].nHe=0;
        cell[k].T=1e4;
